#!/usr/bin/env python3
"""Smart PyMuPDF pipeline for reproducibility auditing of conference papers."""
...
from pipeline_pymupdf_baseline import (
    ...
    setup_logger,
    split_main_text_and_appendix_tail,
    split_paragraphs,
    write_raw_output,
)


DEFAULT_OUTPUT_NAME = "reproducibility_assessments_smart_pymupdf.csv"
DEFAULT_RAW_DIRNAME = "reproducibility_raw_smart_pymupdf"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Assess reproducibility evidence from PDF text using abstract-first plus smart full-text context."
    )
...
def extract_filtered_pdf_text(pdf_path: Path) -> dict[str, Any]:
    full_text = extract_pdf_text(pdf_path)
    main_text, appendix_tail = split_main_text_and_appendix_tail(full_text)
    filtered_text = "\n\n".join(part for part in (main_text, appendix_tail) if part.strip())
    blocks = build_text_blocks(filtered_text)
    kept_headings = [block["heading"] for block in blocks if block["heading"]]
    return {
        "text": filtered_text,
        "full_text": full_text,
        "main_text": main_text,
        "appendix_tail": appendix_tail,
        "kept_headings": kept_headings,
        "dropped_headings": ["references/bibliography tail"] if appendix_tail or main_text != normalize_whitespace(full_text) else [],
        "kept_section_count": len(blocks),
        "dropped_section_count": 1 if appendix_tail or main_text != normalize_whitespace(full_text) else 0,
    }
...
def main() -> None:
    ...
    if not args.api_key:
        raise SystemExit("Missing API key. Pass --api-key or set GROQ_API_KEY.")
    ...
    logger.info(
        "Starting smart PyMuPDF run with conference=%s year=%s max_papers=%s overwrite=%s full_text_threshold=%s",
        args.conference,
        args.year,
        args.max_papers,
        args.overwrite,
    ...
    for sample_csv in sample_csvs:
        ...
        for index, row in enumerate(sampled_rows, start=1):
            ...
            try:
                ...
                filtered_payload = extract_filtered_pdf_text(pdf_path)
                ...
                raw_payload = {
                    ...
                    "full_text_chars": len(filtered_payload["full_text"]),
                    "filtered_text_chars": len(filtered_payload["text"]),
                    "main_text_chars": len(filtered_payload["main_text"]),
                    "appendix_tail_chars": len(filtered_payload["appendix_tail"]),
